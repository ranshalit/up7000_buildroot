...
If `sdk/` is missing or stale, re-create it from the SDK tarball generated by
Buildroot:

```bash
cd /path/to/br_2024/

# Re-generate the SDK tarball
make -C buildroot-2024.02.9 O="$(pwd)/output" sdk
...
If `output/.config` is missing (for example on a new machine), run
`up7000_defconfig` once before `make ... sdk`:
